...
public class Simulator {
	...
	private void processTimeoutEvent(Event e) {
		...
		case OUT:
		case TREATING:
			//timeout arrivato troppo rtardi, non serve più ignoriamolo
			break;
		default:
			throw new IllegalArgumentException("Stato paziente errato"+p.toString());
		}
	}
	...
	private void processTriageEvent(Event e) {
		...
		if(this.occupiedStudios<NS){
			...
			//Se no, lo metto in lista d'attesa e schedulo l'azione di time-out
			int timeout=0;
			if(p.getStatus()==PatientStatus.WHITE)
